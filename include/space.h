...
#define SAVE_SPACE_DATA(save_desc, space_desc, save_file_name, byte_buf_size, byte_buf_name, ...)                                          \
  {                                                                                                                                        \
    ...
    if (!save_desc.is_server_save) {                                                                                                       \
      strcpy(dir_path,                                                                                                                     \
             TextFormat("save/save%d/spaces/%s-%zu/", save_desc.id, space_id_to_name(space_desc.type->space_id), space_desc.id));          \
    } else {                                                                                                                               \
      strcpy(dir_path, TextFormat("server-save/spaces/%s-%zu/", space_id_to_name(space_desc.type->space_id), space_desc.id));              \
    }                                                                                                                                      \
    dir_create(dir_path);                                                                                                                  \
    char path[512];                                                                                                                        \
    if (!save_desc.is_server_save) {                                                                                                       \
      strcpy(path,                                                                                                                         \
             TextFormat("save/save%d/spaces/%s-%zu/" save_file_name ".bin", save_desc.id, space_id_to_name(space_desc.type->space_id),     \
                        space_desc.id));                                                                                                   \
    } else {                                                                                                                               \
      strcpy(path,                                                                                                                         \
             TextFormat("server-save/spaces/%s-%zu/" save_file_name ".bin", space_id_to_name(space_desc.type->space_id), space_desc.id));  \
    }                                                                                                                                      \
...
#define LOAD_SPACE_DATA(save_desc, space_desc, save_file_name, byte_buf_size, byte_buf_name, ...)                                          \
  {                                                                                                                                        \
    ...
    const char *path = TextFormat("save/save%d/spaces/%s-%zu/" save_file_name ".bin", save_desc.id,                                        \
                                  space_id_to_name(space_desc.type->space_id), space_desc.id);                                             \
