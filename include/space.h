...
extern const SpaceDescriptor SPACE_DESC_DEFAULT;
...
#define SAVE_SPACE_DATA(save_desc, space_desc, save_file_name, byte_buf_size, byte_buf_name, ...)                                          \
  {                                                                                                                                        \
    ...
    __VA_ARGS__                                                                                                                            \
    const char *dir_path = TextFormat("save/save%d/spaces/%s", save_desc.id, space_id_to_name(space_desc.space_id));\
    dir_create(dir_path);\
    const char *path = TextFormat("save/save%d/spaces/%s/" save_file_name ".bin", save_desc.id, space_id_to_name(space_desc.space_id));    \
...
void space_create_default(Space *space);
