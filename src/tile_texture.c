    ...
    if (cJSON_IsArray(variants)) {
      ...
      for (int i = 0; i < len; i++) {
        ...
        if (cJSON_IsString(element)) {
          int path_max_len =
              sizeof(ASSETS_DIR) + strlen(element->valuestring) + 1;
          char path[path_max_len];
          snprintf(path, path_max_len, "%s%s", ASSETS_DIR,
                   element->valuestring);
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    if (file_ext != NULL && strcmp(file_ext, ".json") == 0) {
      ...
      if (file_name_len < suffix_len ||
          strcmp(file_path + file_name_len - suffix_len, suffix) != 0) {
