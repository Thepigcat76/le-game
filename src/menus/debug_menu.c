...
static void debug_menu_items_button_clicked() {
  GAME.debug.options.game_object_display = (GAME.debug.options.game_object_display == DEBUG_DISPLAY_ALL_ITEMS)
      ? DEBUG_DISPLAY_NONE
      : DEBUG_DISPLAY_ALL_ITEMS;
}

static void debug_menu_tiles_button_clicked() {
  GAME.debug.options.game_object_display = (GAME.debug.options.game_object_display == DEBUG_DISPLAY_ALL_TILES)
      ? DEBUG_DISPLAY_NONE
      : DEBUG_DISPLAY_ALL_TILES;
}

static void debug_menu_beings_button_clicked() {
  GAME.debug.options.game_object_display = (GAME.debug.options.game_object_display == DEBUG_DISPLAY_ALL_BEINGS)
      ? DEBUG_DISPLAY_NONE
      : DEBUG_DISPLAY_ALL_BEINGS;
}

static void debug_menu_collisions_button_clicked() {
  GAME.debug.options.collisions_enabled = !GAME.debug.options.collisions_enabled;
}

static void debug_menu_hitboxes_button_clicked() {
  GAME.debug.options.hitboxes_shown = !GAME.debug.options.hitboxes_shown;
}

static void debug_menu_tile_info_button_clicked() {
  GAME.debug.options.print_tile_debug_info = !GAME.debug.options.print_tile_debug_info;
}
...
void debug_menu_render(UiRenderer *renderer, const ClientGame *game) {
  ...
  RENDER_BUTTON({.message = "Beings",
                 ...
                 .x_offset = x_offset,
                 .y_offset = y_offset});
  RENDER_BUTTON({.message = GAME.debug.options.collisions_enabled ? "Collisions: On" : "Collisions: Off",
                 .texture = DEBUG_BUTTON_TEXTURE,
                 .selected_texture = DEBUG_BUTTON_SELECTED_TEXTURE,
                 ...
                 .width = 100,
                 .height = 16});
  RENDER_BUTTON({.message = GAME.debug.options.hitboxes_shown ? "Hitboxes: Shown" : "Hitboxes: Hidden",
                 .texture = DEBUG_BUTTON_TEXTURE,
                 .selected_texture = DEBUG_BUTTON_SELECTED_TEXTURE,
                 ...
                 .width = 120,
                 .height = 16});
  RENDER_BUTTON({.message = GAME.debug.options.print_tile_debug_info ? "Print tile info" : "Don't Print tile info",
                 .texture = DEBUG_BUTTON_TEXTURE,
                 .selected_texture = DEBUG_BUTTON_SELECTED_TEXTURE,
