...
void game_render_overlay(Game *game) {
  ...
  if (game->slot_selected) {
    Vec2f mouse_pos = GetMousePosition();
    if (mouse_pos.x + game->texture_manager.textures[TEXTURE_TOOLTIP].width * 5 > GetScreenWidth()) {
      mouse_pos.x -= game->texture_manager.textures[TEXTURE_TOOLTIP].width * 5;
    }
